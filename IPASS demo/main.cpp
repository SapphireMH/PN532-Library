...
int main() {
	...
	for(size_t i = 0; i < known_uid.size(); i++) {
		...
		if( uid == known_uid[i] ) {
			...
			hwlib::cout << "UID found in database!\n";
	...
	if( !card_auth ) {
		...
		hwlib::cout << "UID not in database!\n";
	...
	if( uid == known_uid[1] ) {
		...
		// We do stuff if we have a known uid.
		hwlib::cout << "Access level 2 detected!\n";
